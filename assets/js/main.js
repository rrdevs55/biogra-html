...
(function ($) {
    ...
    var device_width = window.innerWidth;
        ...
        if ($('.side-menu-wrap').hasClass('has---mneu')) {
            $(".mobile-menu-items").meanmenu({
                meanMenuContainer: ".side-menu-wrap",
                meanScreenWidth: "3000", // সব স্ক্রিনেই দেখাবে
                meanMenuCloseSize: "30px",
                meanRemoveAttrs: true,
                meanExpand: ['<i class="fa-solid fa-caret-down"></i>']
            });
    ...
    const hoveritem = document.querySelectorAll(".rr-hover-reveal-item");
        ...
        function moveImage(e, hoveritem, index) {
            ...
            if (hoveritem.children[index]) {
                ...
                img.style.transform = `translate(${x}px, ${y}px) rotate(5deg)`; // ✅ Rotate only the image
            }
        }
        ...
        // প্রথম item-কে active ক্লাস দাও
        if (items.length > 0) {
            items[0].classList.add('active');
        }
    
        // IntersectionObserver দিয়ে active item পরিবর্তন করবো
        const observer = new IntersectionObserver((entries) => {
            ...
            root: null,
            rootMargin: '0px',
            threshold: 0.5 // স্ক্রিনে ৫০% আসলে একটিভ হবে
        });
